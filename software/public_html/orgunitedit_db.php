...
//org status can be terminated/suspended/ or activated again after suspended
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
  ...
  {
    $system_setting = trim($_POST["system_setting"]);
  }
  ...
  if ($update_stmt->execute()) 
  {
    ...
    if($orgunit_status=='Suspended' && $current_status['orgunit_status']!='Suspended')
    {
      ...
      orgunit_id=:orgunit_id AND `Camp_Status`='Active' AND ou_status='Active'");
      ...
      admin.AdminId=tbl_orgunit_user.user_id AND orgunit_id=:orgunit_id AND ou_status='Active'");
      ...
      while($rowu= $stmt_u->fetch())
        ...
        //suspend ou status 
        $stmt3=$conn->prepare("UPDATE tbl_orgunit_user SET ou_status='Org Suspended' WHERE orgunit_id=:orgunit_id AND ou_status='Active'");
        $stmt3->bindValue(':orgunit_id', $id);
        $stmt3->execute();
    ...
    if($orgunit_status=='Terminated'  && $current_status['orgunit_status']!='Terminated')
    {
      ...
      $stmt= $conn->prepare("SELECT * FROM `campaign` INNER JOIN tbl_orgunit_user ON campaign.AdminID = tbl_orgunit_user.user_id AND 
      orgunit_id=:orgunit_id AND ou_status='Active'");
      $stmt->bindValue(':orgunit_id', $id);
      $stmt->execute();
      ...
      //Terminate ou status 
      $stmt3=$conn->prepare("UPDATE tbl_orgunit_user SET ou_status='Org Terminated' WHERE orgunit_id=:orgunit_id AND ou_status='Active'");
      $stmt3->bindValue(':orgunit_id', $id);
      $stmt3->execute();
    ...
    if($orgunit_status=='Active'&& $current_status['orgunit_status']!='Active')
    {
      ...
      //Active ou_status
      $stmt3=$conn->prepare("UPDATE tbl_orgunit_user SET ou_status='Active' WHERE orgunit_id=:orgunit_id AND ou_status='Org Suspended'");
      $stmt3->bindValue(':orgunit_id', $orgunit_id);
      $stmt3->execute();
    }//active end
  }
