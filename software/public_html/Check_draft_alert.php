...
d.alert_iteration
FROM
admin a, campaign c, camp_alerts d, tbl_orgunit_user ou
WHERE
a.AdminID = ou.user_id AND c.ou_id= ou.ou_id AND d.CampID = c.CampID AND  d.alert_iteration=(SELECT max(alert_iteration) from camp_alerts cal where  cal.CampID = :CampID ) AND 
c.CampID = :CampID AND ou_status='Active' ";
...
if ($stmt_get->rowCount() > 0) {
    ...
    // $Journal_title =  $row2['Journal_title'];
    
    $FirstName = html_entity_decode(trim($row_get['fname']));
 ...
 ";
    ...
    // echo "Dear Dr. " . $emails['fname'] . " " . $emails['Lastname']. ":<br/></br/>";
     ...
     $DB_Rows   = [$article_title, $Journal_title, $FirstName, $last_name];
...
WHERE mailserverid=$mailserverid";
    ...
    // from adress of root mailshub will be ussed
