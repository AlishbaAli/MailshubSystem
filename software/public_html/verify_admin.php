...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt1 = $conn->prepare("SELECT camp_alert from camp_alerts 
    WHERE CampID=:CampID 
    AND alert_iteration = (SELECT max(alert_iteration) from camp_alerts where  CampID = :CampID )");

    $stmt1->bindValue(':CampID', $CampID);
    $stmt1->execute();
    $subscription_draft = $stmt1->fetch();
    $subscription_draft = $subscription_draft["camp_alert"];
