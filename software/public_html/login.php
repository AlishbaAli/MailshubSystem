...
if (isset($_SESSION['AdminId'])) {
	...
	//User already logged in. Redirect them  to the index page.
					...
					WHERE username = :username";
						...
						if ($admin === false) {
							...
							if ($validPassword === true) {
								...
								tbl_organizational_unit ON  tbl_organizational_unit.orgunit_id=tbl_orgunit_user.orgunit_id WHERE user_id= $user_id AND
								ou_status='Active'");
...
// all activity session set to no
								//user management code
								$activity="SELECT * From tbl_activity where 1 ";
								$activity=$conn->prepare($activity);
								$activity->execute();
								$activities = $activity->fetchAll();

								foreach($activities as $act){

								$activity = trim($act['activity_code']);
								$_SESSION[$activity] = "NO";
