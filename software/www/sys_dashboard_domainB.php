...
$MS="SELECT  Count(`domain_name`) as urlb,
Count(Case when domain_status='In Active' then `domain_name` end ) as Inactiveorg,
count(Case when domain_status='Active' then `domain_name` end ) as Activeorg

FROM `blocked_domains` WHERE 1";
$MS=$conn->prepare($MS);
$MS->execute();
$ms=$MS->fetch();
...
$query="SELECT *,date(blocked_domains.system_date)as sd FROM blocked_domains left join 
(SELECT blocked_domain_id,blocked_domain_org.orgunit_id,GROUP_CONCAT(orgunit_name SEPARATOR ', ') as orgunit_name
