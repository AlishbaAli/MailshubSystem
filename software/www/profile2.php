...
<body class="theme-blue">
   ...
   ON `tbl_orgunit_user`.`user_id` = $user_id AND tbl_organizational_unit.orgunit_id = tbl_orgunit_user.orgunit_id ");
        $stmt->execute();
        $row = $stmt->fetch();
...
FROM campaign 
inner join admin inner join reply_to_emails inner join mailservers inner join products inner join tbl_orgunit_user
where 
campaign.mailserverid=mailservers.mailserverid 
and campaign.productid = products.productid 
and campaign.ou_id = tbl_orgunit_user.ou_id 
and campaign.rtemid=reply_to_emails.rtemid 
and tbl_orgunit_user.user_id = admin.AdminId 
and tbl_orgunit_user.user_id=:AdminId";
                                ...
                                <table class="table table-responsive table-striped xl-pink">
                                    <thead>
                                        <tr>
                                            <th style="text-align: center;"><i class="btn-secondary icon-check" style="border-radius: 9px;"></i></th>
                                            <th>Campaign Name</th>
