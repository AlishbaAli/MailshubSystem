...
<div class="table-responsive">                    
                                    <table id="display5" class="table  center-aligned-table table-bordered table-hover  dataTable table-custom">
...
$sql = "SELECT orgunit_id,ipdetails.mailserverid as mailserverid,ipdetails.ipdetailid,`ipaddress` ,service_providers.sp_id as service_provider,`ipstatus`,`ipblack_score`,`ipblack_color`, 
...
left join `mailserver-orgunit` on `mailserver-orgunit`.`mailserverid` = ipdetails.mailserverid 
 RIGHT JOIN tbl_ipblacklist_log ON ipdetails.ipdetailid= tbl_ipblacklist_log.ipdetailid  WHERE orgunit_id is not null 
and ( whitelist_date is null and blacklist_duration is null) GROUP BY ipdetailid";
 ...
 $stmt->execute();
    ...
    inner join mailservers on mailservers.mailserverid = ipdetails.mailserverid 
     ...
     left join `mailserver-orgunit` on `mailserver-orgunit`.`mailserverid` = ipdetails.mailserverid
     WHERE orgunit_id=:org  and ( whitelist_date is null and blacklist_duration is null) GROUP BY ipdetailid";
 $stmt = $conn->prepare($sql);
 $stmt->bindParam(':org', $_POST["ms_id"]);
$stmt->execute();
                            ...
                            ?>
                                ...
                                <td><?php 
                                if ($row['orgunit_id'] != null || !empty($row['orgunit_id'])){
                                $oid= $row['orgunit_id'];
                                $stmtm = $conn->prepare("SELECT orgunit_name FROM tbl_organizational_unit WHERE orgunit_id =$oid");
                                $stmtm->execute();
                                $rowm = $stmtm->fetch();
                               echo $rowm["orgunit_name"] . " "; } else {echo " ";} ?></td>
