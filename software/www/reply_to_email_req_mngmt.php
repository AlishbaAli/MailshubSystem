...
<tbody>
  <?php  while ($rowb = $stmtb->fetch()) {
    ...
    $stmtu= $conn->prepare("SELECT user_id FROM tbl_orgunit_user WHERE orgunit_id=:orgunit_id");
 ...
 $admid_string="";
      ...
      $stmtc= $conn->prepare("SELECT  Camp_Status, AdminID,
      COUNT(
...
END
      ...
      FROM campaign WHERE rtemid=:rtemid AND AdminID IN($admid_string)");
