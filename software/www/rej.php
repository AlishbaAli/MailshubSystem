...
$user_role=$conn->prepare($u_role); $user_role->execute(); $user_role=$user_role->fetch();
 ...
 $sql.=" group by camp_draft.CampID";
                                                    ...
                                                    while ($row1=$stmt->fetch()){
                                                        ...
                                                        $sql2 = "SELECT campaign.CampID,CampName,rejected_iteration,
                                                        CampDate,reason, camp_draft.subscription_draft as rej_draft, camp_draft.draft_subject as cdsub,
                                                         draft.draft_subject as dsub, draft.subscription_draft as sub_draft
                                                         FROM camp_draft INNER JOIN campaign on camp_draft.CampID=campaign.CampID  and campaign.Camp_Status='Rejected' and rejected_iteration != '0'
                                                         and campaign.CampID= $cid and rejected_iteration=$rit
                                                         Left JOIN draft on draft.CampID=campaign.CampID ";
                                                          $sql2.=" group by campaign.CampID";
    ...
    if (!empty(trim($result_get['Journal_title']))) { 
                                                        ...
                                                        ?>
                                                                <tr>
                                                                    ...
                                                                    <a href="Edit_camp_new.php?CampID=<?php echo $row["CampID"]; ?>"> <button type="button" class="btn btn-warning" title="Edit">Edit Campaign<!-- <i class="fa fa-edit"></i> --></button></a>
                                                                    
                                                                        <button id="<?php echo $row['CampID'].$row['rejected_iteration']; ?>"  class="btn btn-secondary"  data-toggle="modal" data-target="#exampleModalCenter<?php echo $row['CampID'].$row['rejected_iteration']; ?>"> View Rejected Draft</button>
    
                                                                    <div class="modal fade" id="exampleModalCenter<?php echo $row['CampID'].$row['rejected_iteration']; ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"      aria-hidden="true">
                                                                        <div class="modal-dialog modal-dialog-centered" style="max-width: '100%';min-width: 80%;" role="document">
                                                                            <div class="modal-content"  style="overflow:scroll;">
                                                                               
                                                                                <div class="modal-header">
                                                                                        <h5 class="modal-title" id="exampleModalCenterTitle"><?php echo $row["CampName"]; ?>Rejected Draft </h5>
    ...
    <?php if($result_act['add_draft_activity']!=1 ) { 
       $cidd= $row["CampID"];  
        $cdft="SELECT * from draft where CampID='$cidd'";
        $add=$conn->prepare($cdft);
        $add->execute();
        $add->fetch(); if($add->rowCount()>0){
            ?>
        ...
        <div class="modal-dialog modal-dialog-centered" style="max-width: '100%';min-width: 80%;" role="document">
            <div class="modal-content"  style="overflow:scroll;">
                ...
                <div class="modal-body">
                         ...
                         <div id=""> <?php echo $message_app_view; ?></div>
    ...
    <a target="_blank" href='Edit_draft.php?CampID=<?php echo $row["CampID"]; ?>'><button type='button' class='btn btn-warning'>Edit Draft</button></a> 
<a target="_blank" href='deleteDraft.php?CampID=<?php echo $row["CampID"]; ?>'><button type='button' data-type="confirm" onclick='return deletedraft();' class='btn btn-danger'>Discard Draft</button></a>
        ...
        <?php } else { ?> 
              <a target="_blank" href='addDraft.php?CampID=<?php echo $row["CampID"]; ?>'><button type='button' class='btn btn-primary'>Add Draft</button></a>
                                                                       
             <!--  <a target="_blank" href='viewDraft.php?CampID=<?php echo $row["CampID"]; ?>'><button type='button' class='btn btn-primary'>View Draft</button></a> -->
    <?php } } ?>
    ...
    <?php } ?>
    <?php if($result_act['add_draft_activity']==1 && (trim($message_app)!=trim($message_app_view)) ) {?>
