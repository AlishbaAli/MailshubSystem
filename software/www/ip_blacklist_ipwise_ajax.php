...
<div class="table-responsive">                    
                                    <table id="displayip" class="table  center-aligned-table table-bordered table-hover dataTable table-custom ">
...
$sql = "SELECT mailservers.mailserverid,ipdetails.ipdetailid,`ipaddress` ,service_providers.sp_id as service_provider,`ipstatus`,`ipblack_score`,`ipblack_color`, date(whitelist_date) as whitelist_date ,blacklist_duration,
CONCAT(dnsbl_id,'|',blacklist_date,'|',blacklist_score, '|',blacklist_color) AS dnsbl_id FROM ipdetails
...
";
 ...
 $stmt->execute();
    ...
    inner join mailservers on mailservers.mailserverid = ipdetails.mailserverid 
                                                ...
                                                left join ip_pool on ip_pool.ip_pool_id = ip_addresses.ip_pool_id
                                                left join service_providers on service_providers.sp_id=ip_pool.sp_id
     RIGHT JOIN tbl_ipblacklist_log ON ipdetails.ipdetailid= tbl_ipblacklist_log.ipdetailid WHERE ipdetails.ipdetailid=:service_provider 
     ";
 $stmt = $conn->prepare($sql);
                                                           ...
                                                           <?php } 
                                                            ...
                                                            <span class="badge badge-warning" style="background:yellow; color:black"> <?php  
                                                                ...
                                                                echo $row["ipblack_score"] . " "; ?>
                                                                        ...
                                                                        <?php } ?>
                                                                        <td><?php echo $row["whitelist_date"];?></td>
                                                                        <td><?php if(!empty($row["blacklist_duration"])){ 
                                                                            echo $row["blacklist_duration"]." days";}?></td>
